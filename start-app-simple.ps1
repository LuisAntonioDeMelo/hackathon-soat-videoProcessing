...
Write-Host ""
Write-Host "Aguardando servicos ficarem disponiveis..." -ForegroundColor Yellow
...
while ($elapsed -lt $timeout -and -not $backendReady) {
    try {
        ...
        if ($response.StatusCode -eq 200) {
            $backendReady = $true
            Write-Host "   Backend esta pronto!" -ForegroundColor Green
        }
    } catch {
        Start-Sleep -Seconds 5
        $elapsed += 5
...
if (-not $backendReady) {
    Write-Host "   Backend nao ficou pronto em $timeout segundos" -ForegroundColor Red
...
while ($elapsed -lt $timeout -and -not $frontendReady) {
    try {
        ...
        if ($response.StatusCode -eq 200) {
            $frontendReady = $true
            Write-Host "   Frontend esta pronto!" -ForegroundColor Green
        }
    } catch {
        Start-Sleep -Seconds 5
        $elapsed += 5
...
Write-Host ""
Write-Host "   APLICACAO:" -ForegroundColor Yellow
...
if ($frontendReady) {
    Write-Host "Abrindo aplicacao principal..." -ForegroundColor Yellow
    ...
    Write-Host "Frontend pode nao estar totalmente pronto. Aguarde alguns segundos e acesse:" -ForegroundColor Yellow
    Write-Host "   http://localhost:3000" -ForegroundColor White
}

Write-Host ""
Write-Host "Sistema completo disponivel com:" -ForegroundColor Green
